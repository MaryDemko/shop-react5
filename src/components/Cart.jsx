...
export const Cart = ({ cartItems, onUpdateQuality, onRemoveItem, onSaveCart }) => {
            ...
            {cartItems.length === 0 ? (
                ...
                <>
                        ...
                        {cartItems.map(item => (
                            <div key={item.id} className='flex flex-col md: flex-row items-center justify-between border-b pb-4'>
                                ...
                                <div className='flex items-center my-2 md:my-0'>
                                    <button
                                        onClick={() => onUpdateQuality(item.id, item.quantity - 1)} className='px-3 py-1 border rounded-l disabled:opacity-50 transition-colors'
                                        disabled={item.quantity <= 1}
                                    >
                                        -
                                    </button>
                                    <span className='px-4 border-t border-b'>{item.quantity}
                                    </span>
                                    <button
                                        onClick={() => onUpdateQuality(item.id, item.quantity + 1)}
                    ...
                    <div className='mt-6 text-right'>
                        <p className='text-xl font-semibold text-green-600'>Общая стоимость: {totalPrice.toFixed(2)} ₽
                        </p>
                        {appliedDiscount > 0 && (<p className='text-xl font-semibold text-green-600'>Со Скидкой: {discountPrice.toFixed(2)} ₽
                        </p>
                        )}
                    </div>

                    <div className='mt-6'>
                        <lable htmlFor='promo' className='block text-gray-700 mb-2'>
                            Промокод
                        </lable>
